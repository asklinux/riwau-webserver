...
- Virtual host, reverse proxy, server-side runtime declarations, dan per-host WAF tuning wajib dikonfigurasi melalui SQLite keys `virtual_hosts_enabled`, `virtual_hosts`, `virtual_host_waf_overrides`, dan `reverse_proxy_*`; jangan tambah fail vhost config berasingan.
...
- WAF audit log mesti kekal berstruktur dan redacted: jangan log raw body, query string, cookie, authorization/header values, atau WAF evidence. Gunakan event `rimau_waf_audit` dan route retention/rotation melalui deployment logger sehingga audit sink khusus diterima.
