...
## ADR-0037: Use curl/nghttp2 For TLS ALPN h2 Real-Client Smoke
...
Add an automated CTest named `rimau_tls_alpn_h2_curl` that uses `curl --http2` as a real HTTP/2 client to verify that Rimau's TLS ALPN negotiation selects `h2` when `http2_enabled=true` and `tls_alpn_protocols=h2,http/1.1`.

Implementation:

- The test script is `tests/test_tls_alpn_h2_curl.py`.
- It starts a temporary Rimau server with TLS enabled and a self-signed test certificate generated by the bundled OpenSSL binary from the active build tree.
- It discovers `curl` from `RIMAU_CURL` or `PATH`, checks that `curl -V` reports `HTTP2`, and uses CTest skip semantics if HTTP/2-capable curl is unavailable.
- It parses verbose curl output and fails if the real client does not negotiate ALPN `h2`.
- It accepts both curl 7 and curl 8 verbose wording for confirmed HTTP/2 usage.
- If curl request completion succeeds in the future, the test verifies the response body.
- For the current partial HTTP/2 implementation, the test also accepts the known curl/nghttp2 `COMPRESSION_ERROR` path caused by missing HPACK Huffman decode.

Reason:

The previous TLS ALPN `h2` validation used Python `ssl` plus manually constructed HTTP/2 frames. That proves Rimau's raw path but does not exercise a real HTTP/2 client stack. `curl` with nghttp2 is commonly available in Linux CI and gives a practical ALPN negotiation smoke without bundling another HTTP/2 client dependency into Rimau.
