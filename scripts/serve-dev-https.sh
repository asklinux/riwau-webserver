...
host="${HOST:-127.0.0.1}"
port="${HTTPS_PORT:-18443}"
database_path="${DATABASE:-${build_dir}/rimau-dev-https.sqlite3}"
cert_file="${TLS_CERT_FILE:-certs/rimau-dev.crt}"
key_file="${TLS_KEY_FILE:-certs/rimau-dev.key}"

cmake -S . -B "${build_dir}" -DRIMAU_ENABLE_TESTS=ON
cmake --build "${build_dir}"

TLS_CERT_FILE="${cert_file}" TLS_KEY_FILE="${key_file}" ./scripts/generate-dev-cert.sh
...
"./${build_dir}/rimau-server" --database "${database_path}" --set "tls_min_version=TLSv1.2" >/dev/null
"./${build_dir}/rimau-server" --database "${database_path}" --set "tls_max_version=TLSv1.3" >/dev/null
"./${build_dir}/rimau-server" --database "${database_path}" --set "tls_alpn_protocols=http/1.1" >/dev/null
...
"./${build_dir}/rimau-server" --database "${database_path}" --set "tls_sni_required=false" >/dev/null
"./${build_dir}/rimau-server" --database "${database_path}" --set "tls_certificate_file=${cert_file}" >/dev/null
"./${build_dir}/rimau-server" --database "${database_path}" --set "tls_private_key_file=${key_file}" >/dev/null
"./${build_dir}/rimau-server" --database "${database_path}" --set "tls_enabled=true" >/dev/null

echo "Rimau Web Server running at https://${host}:${port}/"
